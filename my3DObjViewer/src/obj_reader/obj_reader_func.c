...
struct OBJECT_ATTRIBUTES* lookupObject( const char* sym){
	...
	struct OBJECT_ATTRIBUTES* sp = &pSymtab[symhash(sym)%NHASH];
	...
	while(--scount >= 0 ){
		...
		if(++sp >= pSymtab+NHASH) sp = pSymtab;
	}
...
struct ATTRIBUTE_NS* createAttributeNs( float ns ){
	if( !*ppCurrentMtl ){
		yyerror("pCurrent newmtl is not specified.");
		 abort();
	}

	struct ATTRIBUTE_NS* pAttributeNs = (struct ATTRIBUTE_NS* )malloc( sizeof(struct ATTRIBUTE_NS) );
	(*pAttributeNs).ns = ns;
	(**ppCurrentMtl).pAttributeNs = pAttributeNs;
...
struct ATTRIBUTE_KA* createAttributeKa( float x, float y, float z ){
	...
	(*pAttributeKa).y = y;
	(*pAttributeKa).z = z;
	(**ppCurrentMtl).pAttributeKa = pAttributeKa;
...
struct ATTRIBUTE_KD* createAttributeKd( float x, float y, float z ){
	...
	(*pAttributeKd).y = y;
	(*pAttributeKd).z = z;
	(**ppCurrentMtl).pAttributeKd = pAttributeKd;
...
struct ATTRIBUTE_KS* createAttributeKs( float x, float y, float z ){
	...
	(*pAttributeKs).y = y;
	(*pAttributeKs).z = z;
	(**ppCurrentMtl).pAttributeKs = pAttributeKs;
...
struct ATTRIBUTE_KE* createAttributeKe( float x, float y, float z ){
	...
	(*pAttributeKe).y = y;
	(*pAttributeKe).z = z;
	(**ppCurrentMtl).pAttributeKe = pAttributeKe;
...
struct ATTRIBUTE_NI* createAttributeNi( float ni ){
	...
	(*pAttributeNi).ni = ni;
	(**ppCurrentMtl).pAttributeNi = pAttributeNi;
...
struct ATTRIBUTE_D* createAttributeD( float d ){
	...
	(*pAttributeD).d = d;
	(**ppCurrentMtl).pAttributeD = pAttributeD;
...
struct ATTRIBUTE_ILLUM* createAttributeIllum( float illum ){
	...
	(**ppCurrentMtl).pAttributeIllum = pAttributeIllum;
...
const char* createAttributeMapKd( const char* pMapKd ){
	...
	(**ppCurrentMtl).pMapKd = strdup( pMapKd );
	(**ppCurrentMtl).pDirname = dirnameForMaterial; // strduped already. line 133: obj_reader_flex.l(newfile())
	return pMapKd;
}

struct MATERIAL_LIBRARY* lookupMaterialLibrary2( const char* pMaterialLibraryName){

	struct MATERIAL_LIBRARY* mL = &pLibtab[symhash(pMaterialLibraryName)%NHASH];
	...
	while(--scount >= 0 ){
		...
		if(++mL >= pLibtab+NHASH) mL = pLibtab;
	}
...
struct MATERIAL* lookupMaterial( const char* pMaterialName){
	...
	struct MATERIAL* mtl = &pMtltab[symhash(pMaterialName)%NHASH];
	...
	while(--scount >= 0 ){
		...
		if(++mtl >= pMtltab+NHASH) mtl = pMtltab;
	}
...
struct VERTNUM* createVertNum( float x, float y, float z ){
...
struct VERT2NUM* createVert2Num(float u, float v ){
...
struct FACENUM* createFaceNum( enum FaceNumType type, int vertnum, int texnum, int normnum ){
...
struct ATTRIBUTE_USE_MATERIAL_LIST* getAttributeUseMaterialList(struct OBJECT_ATTRIBUTES* pObject) {

	struct ATTRIBUTE_USE_MATERIAL_LIST* pAttributeUseMaterialList = (*pObject).pAttributeUseMaterialList;
	return pAttributeUseMaterialList;
}

int getNumOfAttributeUseMaterials() {

	int numOfAttributeUseMaterials = 0;
...
struct ATTRIBUTE_V* createAttributeV( struct VERTNUM* pVertNum ){
	...
	if( *ppCurrentObj != NULL ){
		struct ATTRIBUTE_V_LIST* pAttributeVList = getLastAttributeV( *ppCurrentObj );
		if( pAttributeVList == NULL ){
			(**ppCurrentObj).pAttributeVList = pAttributeVL;
		}
		else{
...
struct MATERIAL* findMaterial( const char* pUseMaterialName ){
	...
	for( int i = 0; i < NHASH; i++ ){
		if( pMtltab[i].pMaterialName != 0 ){
			if( strcmp( pMtltab[i].pMaterialName, pUseMaterialName ) == 0 ){
...
struct ATTRIBUTE_USE_MATERIAL_LIST* createUseMaterial( char* pUseMaterialName ){
	...
	*ppCurrentUseMaterial = pUseMaterial;
	...
	if( *ppCurrentObj != NULL ){
		struct ATTRIBUTE_USE_MATERIAL_LIST* pLastAttributeUseMaterialList = getLastAttributeUseMaterialList( *ppCurrentObj );
		if( pLastAttributeUseMaterialList == NULL ){
			(**ppCurrentObj).pAttributeUseMaterialList = pAttributeUseMaterialList;
		}
		else{
...
struct ATTRIBUTE_VN* createAttributeVN( struct VERTNUM* pVertNum ){
	...
	if( *ppCurrentObj != NULL ){
		struct ATTRIBUTE_VN_LIST* pAttributeVNList = getLastAttributeVN( *ppCurrentObj );
		if( pAttributeVNList == NULL ){
			(**ppCurrentObj).pAttributeVNList = pAttributeVNL;
		}
		else{
...
struct ATTRIBUTE_VT* createAttributeVT( struct VERT2NUM* pVert2Num ){
	...
	if( *ppCurrentObj != NULL ){
		struct ATTRIBUTE_VT_LIST* pAttributeVTList = getLastAttributeVT( *ppCurrentObj );
		if( pAttributeVTList == NULL ){
			(**ppCurrentObj).pAttributeVTList = pAttributeVTL;
		}
		else{
...
struct ATTRIBUTE_F* createAttributeF( struct FACENUM* pFaceNum ){
	...
	if( *ppCurrentObj != NULL ){
		struct ATTRIBUTE_F_LIST* pLastAttributeFL = getLastAttributeF( *ppCurrentObj );
		if( pLastAttributeFL == NULL ){
			(**ppCurrentUseMaterial).pAttributeFList = pAttributeFL;
		}
		else{
...
struct OBJECT_ATTRIBUTES* createObjectAttributes( char* name ){
	...
	*ppCurrentObj = pObjectAttributes;

	return pObjectAttributes;
}

/*
Obtain a number of total faces holding.
*/
int getNumOfFaces() {

	int numOfFaces = 0;

	struct ATTRIBUTE_USE_MATERIAL_LIST* pUseMaterialList = (**ppCurrentObj).pAttributeUseMaterialList;
	while (pUseMaterialList != NULL) {
...
int getNumOfIndicesPerFace() {
	...
	struct ATTRIBUTE_F_LIST* pFL = getLastAttributeF( *ppCurrentObj );
	if (pFL != NULL) {
...
*/
void getVertexIndiciesPerFaces(unsigned int* pFacesSuzannu) {
...
Obtain only vertex indicies to specified storage.
*/
void getVertexIndicies2PerFaces(unsigned int* pFacesSuzannu, struct ATTRIBUTE_F_LIST* pFL ) {
/*
	struct ATTRIBUTE_USE_MATERIAL_LIST* pUseMaterialList = (*pCurrentObj).pAttributeUseMaterialList;
	while (pUseMaterialList != NULL) {
...
void getVertices( float* pVerticesSuzannu) {
	...
	struct ATTRIBUTE_V_LIST* pVL = (**ppCurrentObj).pAttributeVList;
	while (pVL != NULL) {
...
/*
Obtain all texture coordinates into specified storage.
*/
void getTexCoords(float* pTexCoords) {

	struct ATTRIBUTE_VT_LIST* pVTL = (**ppCurrentObj).pAttributeVTList;
	while (pVTL != NULL) {

		struct ATTRIBUTE_VT* pVT = (*pVTL).pAttributeVT;
		*pTexCoords++ = (*(*pVT).pVert2Num).u;
		*pTexCoords++ = (*(*pVT).pVert2Num).v;
...
/*
Obtain a number of texture coordinates.
*/
int getNumOfTexCoords() {
...
void getNormals(float* pNormalsSuzannu) {
	...
	struct ATTRIBUTE_VN_LIST* pVNL = (**ppCurrentObj).pAttributeVNList;
	while (pVNL != NULL) {
...
Visit the index th item and return the item.
*/
struct ATTRIBUTE_VT* getItemATTRIBUTE_VT_LIST(int index) {
	...
	return (*pVTL).pAttributeVT;
}

/*
Obtain all normals to specified storage after rearrangement to along with order of vertices.
*/
void getNormalsWithRearrange2(float* pNormalsSuzannu, struct USE_MATERIAL* pUseMaterial) {
...
*/
void getTexCoordsWithRearrange(float* pTexCoords) {
		...
		if ((*pUseMaterial).pAttributeFList) {
			...
			while (pFL != NULL) {
				...
				do {
					...
					int srcIndex = (*pFaceNum).texnum - 1;
					int destIndex = (*pFaceNum).vertnum - 1;
					...
					case enumFaceNumType1:
						...
						printf("%d/%d/%d ", (*pFaceNum).vertnum, (*pFaceNum).texnum, (*pFaceNum).normnum);
#endif
						pTexCoordsVT[destIndex].u = (*getItemATTRIBUTE_VT_LIST(srcIndex)).pVert2Num->u;
						pTexCoordsVT[destIndex].v = (*getItemATTRIBUTE_VT_LIST(srcIndex)).pVert2Num->v;
						break;
...
Obtain all texture coordinates to specified storage after rearrangement along with order of vertices.
*/
void getTexCoordsWithRearrange2(float* pTexCoords, struct USE_MATERIAL* pUseMaterial) {
...
void dump_one_object( struct OBJECT_ATTRIBUTES* object ){
	...
	struct ATTRIBUTE_V_LIST* pVL = (*object).pAttributeVList;
	while( pVL != NULL ){
	...
	struct ATTRIBUTE_VT_LIST* pVTL = (*object).pAttributeVTList;
	while( pVTL != NULL ){
	...
	struct ATTRIBUTE_VN_LIST* pVNL = (*object).pAttributeVNList;
	while( pVNL != NULL ){
	...
	int numOfFaces = 0;
	struct ATTRIBUTE_USE_MATERIAL_LIST* pUseMaterialList = (*object).pAttributeUseMaterialList;
	while (pUseMaterialList != NULL) {
...
void dump_all_objects(){
	...
	struct OBJECT_ATTRIBUTES* sp = pSymtab;
